...
def test_write_file_rejects_content_over_max_write_bytes(monkeypatch, call_tool):
...
def test_write_then_read_roundtrip(call_tool):
    call_tool(server.write_file, "hello.txt", "merhaba dunya")
    assert call_tool(server.read_file, "hello.txt") == "merhaba dunya"


def test_read_file_rejects_file_over_max_read_bytes(monkeypatch, call_tool):
...
def test_read_file_missing_file_reports_error(call_tool):
